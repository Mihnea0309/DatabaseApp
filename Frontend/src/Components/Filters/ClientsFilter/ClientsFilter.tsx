...
const ClientsFilter: React.FC = () => {
    ...
    const { insurance } = useParams<{ insurance: string }>();
    ...
    const fetchClients = async () => {
        try {
            const asigurare = 'Nu';
            const response = await fetch(`http://localhost:8080/Clienti/get/filterInsurance/${insurance}`);
            if (response.ok) {
    ...
    return (
        <div>
            <h2>Clientii care au/nu au asigurare</h2>
